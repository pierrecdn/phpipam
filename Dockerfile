...
RUN sed -i /etc/apt/sources.list -e 's/$/ non-free'/ && \
    ...
    apt-get install -y --no-install-recommends fping libcurl4-gnutls-dev libgmp-dev libmcrypt-dev libpng12-dev libfreetype6-dev libjpeg-dev libpng-dev libldap2-dev libsnmp-dev snmp-mibs-downloader && \
